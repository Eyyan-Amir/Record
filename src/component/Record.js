...
class Record extends Component {
  ...
  handleSave = () => {
    ...
    if (error) {
      ...
      let array = items;
      array.push(value);
      this.setState({ items: array , value: {name: "", detail: "", time: "", total: "", advance: "", balance: "" } , search:"" , backUp:items});
    }
  }
  handleClick = () => {
    const { items, backUp, search } = this.state;
      ...
      row.push(
        <tr key={index}>
          ...
          <div className='d-flex justify-content-around mt-2'>
            <button type='button' className='btn btn-primary' onClick={() => this.editHandle(index , item)}>Edit</button>
            <button type='button' className='doneBtn btn btn-success' onClick={() => this.done(index)}>Done</button>
          </div>
        </tr>
      )
    }
    ...
    for (let i in save) {
      ...
      array.push(
        <tr key={i}>
          ...
          <td>{item.advance}</td>
          <td>{item.balance}</td>
          <td onClick={() => this.delete(i)} className="cross">&times;</td>
        </tr>
      )
    }
    return array;
    ...
    return (
      <>
        <div className="Record">
            <InputField value={this.state.value} search ={this.state.search}  handleSearch = {this.handleSearch} handleSubmit= {this.handleSubmit} handleChange={this.handleChange} handleSave= {this.handleSave} handleClick={this.handleClick} showModal = {this.showModal} />
            <HistModal modalValue = {this.state.modalValue} clearModal = {this.clearModal} modalItems = {this.modalItems} hideModal = {this.hideModal}/>
